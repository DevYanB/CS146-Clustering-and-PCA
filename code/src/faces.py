...
def random_init(points, k) :
    ...
    return np.random.choice(points, k, replace=False) #uniqueness is defined by the replace=False flag!
    ### ========== TODO : END ========== ###
...
def cheat_init(points) :
    ...
    for point in points:
        if point.label in clusters_dict.keys() is not None:
            clusters_dict[point.label].append(point)
        else:
            clusters_dict[point.label] = [point]

    for _, cluster in clusters_dict.items():
...
def main() :
    ...
    # show_image(im=X[10])
    # show_image(im=X[100])
    # show_image(im=X[1000])



    # # Actual PCA; getting the eigenfaces (U) and mean
    U,mu = PCA(X)
    # plot_gallery([vec_to_image(U[:, i]) for i in xrange(12)]) # eigenvectors supp'd by eigenvalues

    # # The average face
    # average_face = np.mean(X, axis=0)
    # show_image(im=average_face)

    # # Process of checking dimension of mapped feature's effect on facial "clarity"
    ...
    #
    # clusters = kMeans(points, 3, init='random')
    # clusters = kMedoids(points, 3, init='random')
    ...
    #     t_start = time.time()
    #     current_cluster_set = kMedoids(points, 4, init='random')
    #     t_end = time.time()
    #     scores.append(current_cluster_set.score())
    #     times.append(t_end - t_start)
    #
    # print "For kMedoids: average was {}, min was {}, and max was {}".format(np.mean(np.array(scores)), np.min(np.array(scores)), np.max(np.array(scores)))
    # print "Average time taken was {}".format(np.mean(np.array(times)))
    ...
    plt.title('K-means and K-medoids score V/S Number of Principal Components')
    ...
    #             cluster_set = kMeans(points, 2, init='cheat')
